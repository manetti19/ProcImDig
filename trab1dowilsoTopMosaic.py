...
# rgb = np.dstack([R, G, B])
# converte para float em 0..1 preservando as proporções reais entre canais
rgb = np.dstack([
    R_raw.astype(np.float32) / 255.0,
    G_raw.astype(np.float32) / 255.0,
    B_raw.astype(np.float32) / 255.0
])
plt.imsave("teste_rgb.png", np.clip(rgb, 0, 1))
...
salvar_banda_png(R, "Canal R", "canal_R.png")
salvar_banda_png(G, "Canal G", "canal_G.png")
salvar_banda_png(B, "Canal B", "canal_B.png")
...
salvar_banda_png(C, "Canal C (Ciano)", "canal_C.png")
salvar_banda_png(M, "Canal M (Magenta)", "canal_M.png")
salvar_banda_png(Yc, "Canal Y (Amarelo)", "canal_Y.png")
...
salvar_rgb_png(rgb_recomposta, "RGB Recomposta", "rgb_recomposta.png")
...
# 26) Salva H, S, I
salvar_banda_png(H_01, "Hue (H) normalizado", "canal_H.png")
salvar_banda_png(S, "Saturation (S)", "canal_S.png")
salvar_banda_png(I, "Intensity (I)", "canal_I.png")
...
caminho_ral_csv = r".\ral_classic.csv"
...
plt.axis("off")
plt.savefig("imagem_ral.png", dpi=200, bbox_inches="tight")
plt.close()
...
with rasterio.open("indice_ral.tif", "w", **meta_out) as dst:
    dst.write(idx_ral.astype(np.int32), 1)

# 18) cria um “dicionário” (arquivo texto) para interpretar os índices
with open("indice_ral_legenda.txt", "w", encoding="utf-8") as f:
...
print("- imagem_ral.png (visualização)")
print("- indice_ral.tif (raster com índice RAL por pixel)")
print("- indice_ral_legenda.txt (mapa índice -> código RAL)")
